...
function dayOrNight(sunrise: number, sunset: number, current: number): DayPeriod {
...
export const CurrentWeather: React.FC<Props> = ({ currentWeather, sunrise, sunset }: Props): React.ReactElement => {
  const tableValues: {[key: string]: any} = {}
  const excludes = [
    'description',
    'condition',
    'temp',
  ...
  if (currentWeather) {
    ...
    const {windSpeed, windDirectionHuman, windGust, visibility, pressure} = currentWeather
  ...
  return (
    <div>
      ...
      <Card>
        <i className={`owf owf-5x owf-${currentWeather?.condition}${suffix}`}></i>
        <div>{description}</div>
